...
describe('sorted entity adapter operator tests', () => {
  ...
  beforeAll(() => {
    adapter = createSortedEntityAdapter<SortableTestItem>(e => e.id, (a, b) => b.createdAt.getTime() - a.createdAt.getTime());

    state = { ids: [], entities: {} };
    state = adapter.addOne({ id: '1', name: 'Potato Jr', createdAt: new Date(2016, 8, 12) }, state);
    state = adapter.addOne({ id: '2', name: 'Potato Sr', createdAt: new Date(2015, 10, 22) }, state);
    state = adapter.addOne({ id: '3', name: 'Apple', createdAt: new Date(2016, 8, 25) }, state);
    state = adapter.addOne({ id: '4', name: 'Banana', createdAt: new Date(2018, 2, 24) }, state);
    state = adapter.addOne({ id: '5', name: 'General Kenobi', createdAt: new Date(2016, 11, 3) }, state);
  });
  ...
  describe('removeMany tests', () => {
    ...
    it('should remove entities that return true when applied to the predicate', () => {
      const predicate: Predicate<SortableTestItem> = (e: SortableTestItem) => e.name.startsWith('Potato');
  ...
  describe('updateOne tests', () => {
    ...
    it('should reorder the ids for the sorting attribute was changed', () => {
      const update: Update<SortableTestItem> = { id: '2', changes: { createdAt: new Date(2019, 0, 1) } };
    ...
    it('should not change a thing', () => {
      const update: Update<SortableTestItem> = { id: '7', changes: { name: 'Peñarol Inteligencia' } };
  ...
  describe('upsertOne tests', () => {
    ...
    it('should insert the non existent item', () => {
      const update: SortableTestItem = { id: '7', name: 'Peñarol Inteligencia', createdAt: new Date(2017, 5, 2) };
    ...
    it('should change name of existing item', () => {
      const update: SortableTestItem = { id: '2', name: 'John Sena', createdAt: new Date(2015, 10, 22) };
    ...
    it('should reorder ids for sorting attribute was changed', () => {
      const update: SortableTestItem = { id: '2', name: 'Potato Sr', createdAt: new Date(2019, 0, 1) };
  ...
  describe('map tests', () => {
    ...
    it('should reorder ids for sorting attribute was changed in many entities', () => {
      const predicate = (e: SortableTestItem) => e.createdAt.getFullYear() === 2016 ? ({
        ...e,
        createdAt: new Date(e.createdAt.getFullYear() + 3, e.createdAt.getMonth(), e.createdAt.getDate())
      }) : e;
